...
def scrape_top_250(url):
	...
	for a in all_a:
		if next_is_title:
			next_is_title=False
			if '/title/' in a.source:
				#print a.source.split('href"')[0]
				titles.append("http://www.imdb.com" + a.attributes.get('href'))
...
def scrape_movie_page(dom):
	...
	# Commenting felt a bit unnecessary as they are all searching for the right tags and classes.
	# There are a view different levels of difficulty in finding the right data, but no complicated pieces of code.
	# One note is that I used comma as a serarator instead of semicolon, because the csv files did not align.
	# The genres for example are divided among the columns, setting the second genre in the director(s) column.

	# Title
	title_year = dom_body.get_elements_by_tagname("h1")[1]
	title = title_year.content.split("&nbsp")[0]

	# Year
	year = title_year.get_elements_by_tagname("a")[0].content

	# Duration
	duration = dom_body.get_elements_by_tagname("time")[0]
	duration = duration.content.replace("  ","").replace("\n","")

	# Genre
	genres = ""
	genre = dom_body.get_elements_by_classname("itemprop")
	for i in genre:
		if i.get_elements_by_tagname('a') == []:
			i_source = i.source
			if 'itemprop="genre"' in i_source:
				genres = genres + i.content + ","
	genres = genres[:-1]

	# Directors, Writers, Actors
	people = dom_body.get_elements_by_tagname("div")
	people = dom_body.get_elements_by_classname("credit_summary_item")
	directors = ""
	writers = ""
	stars = ""
	for i in people:
		persons_job = i.get_elements_by_tagname("h4")[0].content
		persons = i.get_elements_by_tagname("a")
		for person in persons:
			spec_person = person.get_elements_by_tagname("span")
			if spec_person != []:
				if persons_job == "Director:":
					directors = directors + spec_person[0].content + ","
				elif persons_job == "Writers:":
					writers = writers + spec_person[0].content + ","
				elif persons_job == "Stars:":
					stars = stars + spec_person[0].content + ","
	directors = directors[:-1]
	writers = writers[:-1]
	actors = stars[:-1]

	# Rating
	ratings = dom_body.get_elements_by_classname("ratingValue")
	for i in ratings:
		rating = i.get_elements_by_tagname("span")[0].content

	# Number of Ratings
	ratings_num = dom_body.get_elements_by_tagname("a")
	for i in ratings_num:
		i_small = i.get_elements_by_classname("small")
		if i_small:
			n_ratings = i_small[0].content
